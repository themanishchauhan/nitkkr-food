//#region node_modules/meilisearch/dist/index.js
var n = {
...
//#endregion
//#region src/lib/meilisearch.ts
var MeiliSearchClass = E;
var meiliInstance = null;
function getMeiliClient() {
	if (!meiliInstance) meiliInstance = new MeiliSearchClass({
		host: "http://localhost:7700",
		apiKey: "masterKey"
	});
	return meiliInstance;
}
var MENU_INDEX = "menu_items";
async function searchMenuItems(query, options) {
	const result = await getMeiliClient().index(MENU_INDEX).search(query, {
...
async function setupIndex() {
	await getMeiliClient().index(MENU_INDEX).updateSettings({
...
async function syncAllMenuItems(items) {
	await getMeiliClient().index(MENU_INDEX).addDocuments(items);
	console.log(`Synced ${items.length} menu items to Meilisearch`);
}
//#endregion
export { setupIndex as n, syncAllMenuItems as r, searchMenuItems as t };
